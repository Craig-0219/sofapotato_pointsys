...
function ActivateBridge()
    ...
    -- Override GetIdentifier to use sp_bridge's GetCharacterId (citizenid)
    -- so that sp_recharge uses the same identifier as sofapotato_shop.
    if GetResourceState('sp_bridge') == 'started' then
        local originalGetIdentifier = Bridge.GetIdentifier
        Bridge.GetIdentifier = function(source)
            local ok, cid = pcall(function()
                return exports['sp_bridge']:GetCharacterId(source)
            end)
            if ok and type(cid) == 'string' and cid ~= '' then
                return cid
            end
            return originalGetIdentifier(source)
        end
        print('[sp-recharge] sp_bridge identifier override active (citizenid mode)')
    end
end
